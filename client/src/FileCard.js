...
const FileCard = ({ file, onUpdate, onDelete }) => {
    const handleDownload = async () => {
        try {
          const response = await axios.get(`https://enotes2.vercel.app/download/${file.filename}`, {
  ...
  return (
    <div className='body'>
      ...
      <Card.Body>
        ...
        <Card.Text>
        <a href={`https://enotes2.vercel.app/uploads/${file.filename}`} className="fileName"target="_blank" rel="noopener noreferrer">
        {file.filename}
      </a>
        </Card.Text>
